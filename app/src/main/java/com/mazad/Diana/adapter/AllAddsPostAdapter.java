...
public class AllAddsPostAdapter  extends RecyclerView.Adapter<AllAddsPostAdapter.VH> {
    Context context ;
    List<DirectAddResponse> addsList;

    public AllAddsPostAdapter(Context context, List<DirectAddResponse> addsList) {
    ...
    public void onBindViewHolder(@NonNull VH holder, int position) {
        ...
        if (response.getImg1()!=null){
            Picasso.get().load(AppConstant.BASE_IMAGE+response.getImg1()).into(holder.imgeAdIv);
        }else if (response.getImg2()!=null){
            Picasso.get().load(AppConstant.BASE_IMAGE+response.getImg2()).into(holder.imgeAdIv);
        }else {
            Picasso.get().load(AppConstant.BASE_IMAGE+response.getImg3()).into(holder.imgeAdIv);

        }

        holder.dateTv.setText(response.getTimePublish());
        holder.addressTv.setText(response.getUsers().getCountrys().getName());
        if (response.getIsbooked()==UNBOOKED){
            holder.bookedIv.setVisibility(View.GONE);
        }else if (response.getIsbooked()==BOOKED){
            holder.bookedIv.setVisibility(View.VISIBLE);
        }else {
            holder.bookedIv.setVisibility(View.GONE);
        }
