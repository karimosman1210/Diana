...
public class ListDirectAddsPresenter extends BasePresenter<ListDirectAddsView> {
    void directAdds(Context context,int saleTypeId,int catId){
        view.showloadingviewBase(context);
        MainApi.listAddsApi(saleTypeId,catId,new ConnectionListener<MainResponse<List<DirectAddResponse>>>() {
            @Override
            public void onSuccess(ConnectionResponse<MainResponse<List<DirectAddResponse>>> connectionResponse) {
                ...
                if (connectionResponse.data.success) {
                    if (connectionResponse.data.data != null) {
                        view.DirectPostsList(connectionResponse.data.data);
                    } else
